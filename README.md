...
- **Daily lessons** — a 20-day structured HSK-1-style course, then automatic review days. Each day is a real lesson built around 5 new words: a **grammar pattern** with explanation and examples (吗 questions, 的 possession, 有, 我要, prices, telling time, 不 negation, 很 + adjective, 太…了, 在 location, 坐 transport…), a **dialogue** to play and act out aloud, and 6 tasks — study the words, learn the grammar, practice the dialogue, **speak 2 full sentences at 60%+**, pass a mini-quiz (80%+), and type the pinyin (with tones) for 3 words. A new day unlocks each calendar day, and you can set your **exam date** for a countdown.
- **Speaking practice with grading** — tap 🎤 Record & grade: your voice is recorded (replayable, so you can compare with the native audio) while a Chinese speech recognizer transcribes you. Your attempt is scored against the target characters with per-character right/wrong highlighting and a best-score history per word. Chrome/Edge use the built-in `SpeechRecognition` backend; browsers that block it (Arc, Brave, Safari) automatically switch to an on-device Whisper model via transformers.js — a one-time ~80 MB download that's cached and runs locally, so your voice never leaves your machine.
- **Pinyin typing** — see the characters, type the pinyin with tone numbers (你好 → `ni3 hao3`; neutral tone = 5 or 0, `v` = ü) on a built-in on-screen keyboard with tone keys. Right sounds + right tones = 100%, right sounds but wrong/missing tones = 50%.
- **Training drills** — three 10-round drills for the skills exams and conversations actually test: **Listening** (hear a word or sentence with no text, pick the meaning), **Say it in Chinese** (English prompt → speak the Mandarin from memory, graded by the speech engine, with an optional hint), and **Tone ears** (hear a two-syllable word, identify its tone pattern). Best scores tracked in Progress.
